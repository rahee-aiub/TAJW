...
namespace DataAccessLayer.Helper
{
    ...
    public sealed class SqlHelper
    {
        ...
        //SAAIF:20120318
        //As far as I can't see there's no method to execute a SP with default parameters.
        //So that method is added to execute SP with default parameters.
        public static int ExecuteSPWithDefaultValues(string connectionString, string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                SqlParameter[] commandParameters = new SqlParameter[parameterValues.Length];
                int index = 0;
                foreach (object param in parameterValues)
                {
                    commandParameters[index] = (SqlParameter)param;
                    index++;
                }
        ...
        /// <summary>
        /// Added By Furkan
        /// Execute a stored procedure via a SqlCommand (that returns the IID) against the specified 
        /// SqlTransaction using the provided parameter values.  This method will query the database to discover the parameters for the 
        /// stored procedure (the first time each stored procedure is called), and assign the values based on parameter order.
        /// </summary>
        /// <remarks>
        /// This method returns IID as output parameter.
        /// 
        /// e.g.:  
        ///  long result = ExecuteNonQuery(conn, trans, "PublishOrders", 24, 36);
        /// </remarks>
        /// <param name="transaction">A valid SqlTransaction</param>
        /// <param name="spName">The name of the stored procedure</param>
        /// <param name="parameterValues">An array of objects to be assigned as the input values of the stored procedure</param>
        /// <returns>An long representing the inserted IID affected by the command</returns>
        public static long ExecuteNonQueryForTransaction(SqlTransaction transaction, string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                // Call the overload that takes an array of SqlParameters
                if (ExecuteNonQuery(transaction, CommandType.StoredProcedure, spName, true, commandParameters) > 0)
                {
                    return Convert.ToInt64(commandParameters[0].Value);
                }
                else
                {
                    return -1;
        ...
        /// <param name="commandText">The stored procedure name or T-SQL command</param>
        /// <returns>An int representing the number of rows affected by the command</returns>
        //public static int ExecuteNonQuery(SqlTransaction transaction, CommandType commandType, string commandText, bool hasOutput)
        //{
        //    // Pass through the call providing null for the set of SqlParameters
        //    return ExecuteNonQuery(transaction, commandType, commandText, hasOutput, (SqlParameter[])null);
        //}


        /// <summary>
        /// Added By Furkan
        /// Execute a SqlCommand against the specified SqlTransaction
        /// using the provided parameters.
        /// </summary>
        /// <remarks>
        /// e.g.:  
